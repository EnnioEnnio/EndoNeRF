...
def batchify(fn, chunk):
    ...
    def ret(inputs_pos, inputs_time):
        ...
        for i in range(0, num_batches, chunk):
            out, dx = fn(
                inputs_pos[i:i+chunk], [inputs_time[0][i:i+chunk], inputs_time[1][i:i+chunk]])
            out_list += [out]
            dx_list += [dx]
...
def run_network(inputs, viewdirs, frame_time, fn, embed_fn, embeddirs_fn, embedtime_fn, netchunk=1024*64,
    ...
    assert len(torch.unique(frame_time)
               ) == 1, "Only accepts all points from same time"
...
def render(H, W, focal, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    if use_viewdirs:
        ...
        viewdirs = viewdirs / \
            (torch.norm(viewdirs, dim=-1, keepdim=True) + 1e-6)
...
def render_path(render_poses, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    for i, (c2w, frame_time) in enumerate(zip(tqdm(render_poses), render_times)):
        rgb, disp, acc, _ = render(
            H, W, focal, chunk=chunk, c2w=c2w[:3, :4], frame_time=frame_time, **render_kwargs)
        ...
        if savedir is not None:
            ...
            filename = os.path.join(
                save_dir_estim, '{:03d}.rgb.png'.format(i+i_offset))
            imageio.imwrite(filename, rgb8_estim)

            if save_also_gt:
                rgb8_gt = to8b(gt_imgs[i])
                filename = os.path.join(
                    save_dir_gt, '{:03d}.rgb.png'.format(i+i_offset))
                imageio.imwrite(filename, rgb8_gt)

            if save_depth:
                depth_estim = (1.0 / (disps[-1] + 1e-6)) * \
                    (near_far[1] - near_far[0])
                filename = os.path.join(
                    save_dir_estim, '{:03d}.depth.npy'.format(i+i_offset))
...
def create_nerf(args):
    ...
    if args.use_viewdirs:
        embeddirs_fn, input_ch_views = get_embedder(
            args.multires_views, 3, args.i_embed)
    ...
    if args.do_half_precision:
        ...
        if model_fine is not None:
            [model, model_fine], optimizers = amp.initialize(
                [model, model_fine], optimizer, opt_level='O1')
        else:
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    depth_map = torch.sum(
        weights * z_vals * torch.norm(rays_d[..., None, :], dim=-1), -1)
    disp_map = 1./torch.max(1e-10 * torch.ones_like(depth_map),
                            depth_map / (torch.sum(weights, -1) + 1e-6))
...
def render_rays(ray_batch,
    ...
    near, far, frame_time = bounds[...,
                                   0], bounds[..., 1], bounds[..., 2]  # [-1,1]
    z_samples = None
    ...
    if z_vals is None:
        ...
        if N_importance <= 0:
            raw, position_delta = network_query_fn(
                pts, viewdirs, frame_time, network_fn)
            ...
            z_samples = importance_sampling_ray(
    ...
    if N_importance > 0:
        ...
        if z_samples is not None:
            ret['z_std'] = torch.std(
                z_samples, dim=-1, unbiased=False)  # [N_rays]
...
def train():
    ...
    if args.dataset_type == 'blender':
        ...
        i_train = np.array([i for i in np.arange(int(images.shape[0])) if (
            i not in args.skip_frames)])  # use all frames for reconstruction
        ...
        close_depth, inf_depth = np.ndarray.min(
            bds) * .9, np.ndarray.max(bds) * 1.
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, nerf_model_extras = create_nerf(
        args)
    global_step = start
    ...
    if depth_maps is not None:
        close_depth, inf_depth = np.percentile(
            depth_maps, 3.0), np.percentile(depth_maps, 99.9)
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}'.format(
                'test' if args.render_test else ('path_%s' % args.llff_renderpath), start))
            ...
            imageio.mimwrite(os.path.join(testsavedir, 'video.mp4'), to8b(
                rgbs), fps=args.video_fps, quality=8)
    ...
    if masks is not None:
        ...
        if nerf_model_extras['ray_importance_maps'] is None:
            ...
            ray_importance_maps = torch.Tensor(
                nerf_model_extras['ray_importance_maps']).to(device)
    if depth_maps is not None:
        if nerf_model_extras['depth_maps'] is None:
            depth_maps = torch.Tensor(depth_maps).to(device)
        else:
            depth_maps = torch.Tensor(
                nerf_model_extras['depth_maps']).to(device)
    ...
    for i in trange(start, N_iters):
            ...
            if N_rand is not None:
                ...
                if masks is None or args.no_mask_raycast:
                    ...
                    select_inds, _, cdf = importance_sampling_coords(
                        ray_importance_map[coords[:, 0].long(), coords[:, 1].long()].unsqueeze(0), N_rand)
                    select_inds = torch.max(
                        torch.zeros_like(select_inds), select_inds)
                    select_inds = torch.min(
                        (coords.shape[0] - 1) * torch.ones_like(select_inds), select_inds)
                ...
                target_s = target[select_coords[:, 0],
                                  select_coords[:, 1]]  # (N_rand, 3)
                if depth_maps is not None:
                    depth_s = depth_map[select_coords[:, 0],
                                        select_coords[:, 1]]
                    if not args.no_ndc:
        ...
        if args.add_tv_loss:
            ...
            frame_time_next = times[img_i +
                                    1] if img_i < times.shape[0] - 1 else None
            ...
            if frame_time_prev is not None:
                rand_time_prev = frame_time_prev + \
                    (frame_time - frame_time_prev) * torch.rand(1)[0]
            ...
            if frame_time_next is not None:
                rand_time_next = frame_time + \
                    (frame_time_next - frame_time) * torch.rand(1)[0]
        ...
        if args.add_tv_loss:
            if frame_time_prev is not None:
                tv_loss += ((extras['position_delta'] -
                            extras_prev['position_delta']).pow(2)).sum()
                if 'position_delta_0' in extras:
                    tv_loss += ((extras['position_delta_0'] -
                                extras_prev['position_delta_0']).pow(2)).sum()
            if frame_time_next is not None:
                tv_loss += ((extras['position_delta'] -
                            extras_next['position_delta']).pow(2)).sum()
                if 'position_delta_0' in extras:
                    tv_loss += ((extras['position_delta_0'] -
                                extras_next['position_delta_0']).pow(2)).sum()
        ...
        if not args.no_depth_refine and depth_maps is not None and i % args.depth_refine_period == 0 and refinement_round <= args.depth_refine_rounds:
            ...
            refinement_save_path = os.path.join(
                basedir, expname, 'refinement{:04d}'.format(refinement_round))
            if not os.path.exists(refinement_save_path):
                os.makedirs(refinement_save_path)
            depth_prev_save_path = os.path.join(
                refinement_save_path, 'depth_prev')
            depth_refined_save_path = os.path.join(
                refinement_save_path, 'depth_refined')
            ...
            # Comment HERE from Line 1172 until 1235 to bypass Holefilling
            with torch.no_grad():
            ...
            # Comment from line 1172 until here to bypass Holefilling process
        ...
        if i % args.i_img == 0:
            ...
            writer.add_image('gt', to8b(target.cpu().numpy()),
                             i, dataformats='HWC')
            writer.add_image('rgb', to8b(rgb.cpu().numpy()),
                             i, dataformats='HWC')
            writer.add_image('disp', disp.cpu().numpy(), i, dataformats='HW')
            writer.add_image('acc', acc.cpu().numpy(), i, dataformats='HW')

            if 'rgb0' in extras:
                writer.add_image('rgb_rough', to8b(
                    extras['rgb0'].cpu().numpy()), i, dataformats='HWC')
            if 'disp0' in extras:
                writer.add_image(
                    'disp_rough', extras['disp0'].cpu().numpy(), i, dataformats='HW')
            if 'z_std' in extras:
                writer.add_image(
        ...
        if i % args.i_video == 0:
            ...
            with torch.no_grad():
                savedir = os.path.join(
                    basedir, expname, 'frames_{}_{}_{:06d}_time/'.format(expname, args.llff_renderpath, i))
                rgbs, disps = render_path(
                    render_poses, render_times, hwf, args.chunk, render_kwargs_test, savedir=savedir)
            print('Done, saving', rgbs.shape, disps.shape)
            moviebase = os.path.join(basedir, expname, '{}_{}_{:06d}_'.format(
                expname, args.llff_renderpath, i))
            imageio.mimwrite(moviebase + 'rgb.mp4', to8b(rgbs),
                             fps=args.video_fps, quality=8)
            imageio.mimwrite(moviebase + 'disp.mp4', to8b(disps /
                             np.max(disps)), fps=args.video_fps, quality=8)
